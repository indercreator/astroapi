...
app = FastAPI(title="AajKyaHai Astro API", version="1.0")

# Use ephemeris files from ./ephe folder (place sepl_18.se1 & semo_18.se1 here)
...
def astro(
    ...
    # Use 06:00 local time to represent "day" consistently
    local_tz = pytz.timezone(tz)
    dt_local = local_tz.localize(datetime.strptime(date, "%Y-%m-%d") + timedelta(hours=6))

    # Convert to UTC for swe_calc_ut
    ...
    return {
        "date": date,
        "timezone": tz,
        "sun": {"longitude": round(sun_lon, 6), "sign_hi": sun_hi, "sign_en": sun_en},
        "moon": {"longitude": round(moon_lon, 6), "sign_hi": moon_hi, "sign_en": moon_en},
        "panchang": {
            "tithi": {"hi": tithi_hi, "en": tithi_en},
            "nakshatra": {"hi": nak_hi, "en": nak_en},
        }
    }
